	...
	function displayTeamspeakEx($settings) {
		...
		if ($serverInfo["queryerror"] != 0) {
			...
			foreach($serverInfo["channellist"] as $channelInfo) { if ($channelInfo["parent"] == -1) {
				...
				// Display channel:
				echo("<table><tr><td>");
				echo("<img src=\"inc/teamspeakdisplay/treeimage" . ((($counter + 1) == $currentchannels) ? "3" : "2") . ".png\" alt=\"\"/>");
				echo("<img src=\"inc/teamspeakdisplay/channel.png\" alt=\"\" title=\"" . $popupInfo . "\"/>");
				...
				foreach($serverInfo["playerlist"] as $playerInfo) {
					...
					if ($playerInfo["channelid"] == $channelInfo["channelid"]) {
						...
						// Display player:
						echo("<table><tr><td>");
						echo("<img src=\"inc/teamspeakdisplay/treeimage" . ((($counter + 1) == $currentchannels) ? "4" : "1") . ".png\" alt=\"\"/>");
						echo("<img src=\"inc/teamspeakdisplay/treeimage" . ((($counter_playerandsubchannels + 1) == $currentplayersandsubchannels) ? "3" : "2") . ".png\" alt=\"\"/>");
						echo("<img src=\"inc/teamspeakdisplay/player_" . $playerInfo["displayimage"] . ".png\" alt=\"" . $playerInfo["displayimage"] . "\" title=\"" . $popupInfo . "\"/>");
				...
				foreach($serverInfo["channellist"] as $subchannelInfo) { if ($subchannelInfo["parent"] == $channelInfo["channelid"]) { 
					...
					foreach($serverInfo["playerlist"] as $playerInfo) {
						...
						if ($playerInfo["channelid"] == $subchannelInfo["channelid"]) {
							...
							echo("<table><tr><td>");
							echo("<img src=\"inc/teamspeakdisplay/treeimage" . ((($counter + 1) == $currentchannels) ? "4" : "1") . ".png\" alt=\"\"/>");
							echo("<img src=\"inc/teamspeakdisplay/treeimage" . ((($counter_playerandsubchannels + 1) == $currentplayersandsubchannels) ? "4" : "1") . ".png\" alt=\"\"/>");
							echo("<img src=\"inc/teamspeakdisplay/treeimage" . ((($counter_player + 1) == $currentplayers) ? "3" : "2") . ".png\" alt=\"\"/>");
							echo("<img src=\"inc/teamspeakdisplay/player_" . $playerInfo["displayimage"] . ".png\" alt=\"" . $playerInfo["displayimage"] . "\" title=\"" . $popupInfo . "\"/>");
							echo("</td><td class=\"teamspeakplayer\" title=\"" . $popupInfo . "\">");
							echo(str_replace(" ", "&nbsp;", htmlspecialchars($playerInfo["displayname"])));
							echo("</td></tr></table>\n");
